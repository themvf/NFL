    ...
    if metrics_df.empty:
        ...
        if not all([h_col, a_col, hs_col, as_col]):
            ...
            if teams and team_a in agg["team"].values and team_b in agg["team"].values:
                ...
                st.markdown("---")
                st.markdown("### Players by Position (QB / RB / WR)")
                ...
                if pw_all.empty:
                    ...
                    with c1:
                        ...
                        st.dataframe(qb.sort_values(["team", "pass_yards"], ascending=[True, False]), use_container_width=True)
                    with c2:
                        st.write("RB - Rushing Yards")
                        st.dataframe(rb.sort_values(["team", "rush_yards"], ascending=[True, False]), use_container_width=True)
                    with c3:
                        st.write("WR/TE - Receiving Yards")
                        st.dataframe(wr.sort_values(["team", "rec_yards"], ascending=[True, False]), use_container_width=True)
